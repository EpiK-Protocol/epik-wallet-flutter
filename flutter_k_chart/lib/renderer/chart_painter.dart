...
//    canvas.drawCircle(Offset(x, y), 2.0, paintX);
  ...
  void drawRealTimePrice(Canvas canvas, Size size) {
    ...
    if (tp.width < max) {
      ...
      double right = mWidth-1;
      double top = y - tp.height / 2 - padding;
      double left = right - tp.width-padding * 2 - triangleWidth -padding;
      ...
      canvas.drawRRect(rectBg1, realTimePaint..color = ChartColors.realTimeBgColor_right);
      tp = getTextPainter(format(point.close), color: ChartColors.realTimeTextColor);
      Offset textOffset = Offset(right - padding-tp.width, y - tp.height / 2);
      tp.paint(canvas, textOffset);
      //画三角
      Path path = Path();
      double dx =  textOffset.dx -padding;
