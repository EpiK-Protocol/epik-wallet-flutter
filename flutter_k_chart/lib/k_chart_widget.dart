...
class KChartWidget extends StatefulWidget {
  ...
  KChartWidget(this.datas, {this.mainState = MainState.MA,this.volState = VolState.VOL, this.secondaryState = SecondaryState.MACD, this.isLine,int fractionDigits = 2,this.infoNames}){
...
class _KChartWidgetState extends State<KChartWidget>  with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onScaleUpdate: (details) {
        ...
        mScaleX = (_lastScale * details.scale).clamp(0.25, 3.0);// 最大最小缩放范围
        notifyChanged();
      },
      onScaleEnd: (_) {
        isScale = false;
  ...
  Widget _buildInfoDialog() {
    return StreamBuilder<InfoWindowEntity>(
        ...
        builder: (context, snapshot) {
          ...
          List<String> inlist = widget.infoNames?? infoNames; 
          ...
          return Align(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: List.generate(inlist.length, (i) => _buildItem(infos[i].toString(), inlist[i])),
              ),
            ),
          );
        });
  ...
  Widget _buildItem(String info, String infoName) {
    ...
    if (info.startsWith("+"))
      color = const Color(0xff4ADE2C);//Colors.green.shade600;
    else if (info.startsWith("-"))
      color =const Color(0xffF24F30);//Colors.pinkAccent;
    else
      color = ChartColors.markerTextColor;//Colors.white;
    return Container(
