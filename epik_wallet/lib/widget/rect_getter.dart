...
typedef void RenderCallback(Duration timeStamp, Rect rect, bool isFirst);
...
class RectGetter extends StatefulWidget {
  ...
  RectGetter({@required this.key, @required this.child, this.callback})
      : super(key: key);
...
class _RectGetterState extends State<RectGetter> {
  ...
  @override
  void didUpdateWidget(RectGetter oldWidget) {
  ...
  void onAfterRendering(Duration timeStamp) {
    try {
      if (widget.callback != null) {
        widget.callback(timeStamp, widget.getRect(),isFirst);
      }
    } catch (e) {
      print("RectGetter onAfterRendering error");
      print(e);
    }
    isFirst = false;
  }
